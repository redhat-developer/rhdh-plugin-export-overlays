...
export class TektonSupportHelper {
  ...
  async goToRedHatDeveloperHubProject(): Promise<void> {
    await this.goToByName("Red Hat Developer Hub");
  }
  ...
  async goToRedHatDeveloperHubProjectCITab(): Promise<void> {
    await this.goToRedHatDeveloperHubProject();
    await this.clickTab("CI");
    await this.waitForPipelineRunsOrKubernetesError();
  }

  async searchCatalog(s: string): Promise<void> {
    await this.searchField.clear();
    const baseUrl = process.env.RHDH_BASE_URL ?? process.env.BASE_URL ?? "";
    const searchResponse = this.page.waitForResponse(
      new RegExp(
        `${baseUrl.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}/api/catalog/entities/by-query`,
      ),
    );
  ...
  tableRow(content: string): Locator {
