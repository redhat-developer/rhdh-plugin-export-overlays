...
1. **Workspaces** (`workspaces/*/`) — Define *how to build* dynamic plugin OCI images. Each workspace maps to an upstream source repo (monorepo or standalone). Key files:
   ...
   - `patches/*.patch` — Unified diffs applied to the workspace source before build (lexicographic order; `0-cve-yarn-lock.patch` for CVE lockfile bumps)
...
- **Patch** (`patches/*.patch`): Applies line-by-line changes to workspace source before build. Used for workspace-wide fixes. Export applies `*.patch` in **lexicographic order**. Use `0-cve-yarn-lock.patch` for transitive CVE lockfile fixes (see `scripts/yarnlock-backport/` and `user-guide/06-patch-management.md`); number other patches (`1-`, `2-`, …) to control order.
...
- `scripts/yarnlock-backport/` — CLI to generate `0-cve-yarn-lock.patch` + `cve-backports.yaml` for transitive CVE lockfile backports (see `user-guide/06-patch-management.md`)
