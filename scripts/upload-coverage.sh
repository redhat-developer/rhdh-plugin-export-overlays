...
# source.json SHA. The report was `state: complete` with 24 sessions and it
# still moved:
#
...
#
# The 0.00% flag has a different cause, and it is not path prefixes either — the
# test used correct paths and its `e2e-orchestrator` flag ALSO reads 0.0%, right
# beside the pilot's `e2e-theme`. The flags page reports each flag's coverage on
# the DEFAULT BRANCH. Both uploads targeted a historical commit and never the
# default-branch HEAD, so both read 0.0% there regardless of path correctness.
#
...
# a checkout of the source repo resolved 151/151 for orchestrator. See
# RHIDP-13411.
#
