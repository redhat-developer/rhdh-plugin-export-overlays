...
### Reviewing Scripts That Gate on Structured Pipeline Data

Scripts in `scripts/` sometimes consume structured data from internal pipelines (NFS readiness report, catalog index, smoke test results) and gate CI based on the classification. When reviewing such scripts, verify that the gate's blocking logic can actually fire against real data — not just that the code is internally correct.

**General review criteria:**

1. **Verify value reachability.** When a script classifies items by specific values (statuses, categories, types) and exits non-zero or blocks CI when certain values are found, check whether the upstream data source actually produces all values the blocking logic depends on. A gate that depends on values the pipeline never produces is inert — it provides false confidence that a check is active without catching anything.
2. **Distinguish forward-compatibility from blocking logic.** Handlers for currently unreachable values are acceptable as defensive code (e.g., a `case` branch for a future status). However, blocking verdicts (non-zero exit, CI failures, PR-blocking labels) must not depend solely on values that the current data pipeline cannot produce. If a gate's only blocking path runs through unreachable values, it adds CI overhead without providing signal.
3. **Test against current data.** When a gate would produce zero blocking verdicts against all current data (i.e., the gate is inert), flag this as a correctness concern. An inert gate is worse than no gate — it creates the appearance of validation without performing any. Ask: "If the condition this gate checks for existed today, would this script actually catch it?"
4. **Check the pipeline, not just the schema.** The set of values a pipeline tool actually produces may be narrower than what the output schema allows. Review the upstream tool's behavior (how it populates fields, what inputs it transforms) rather than only reading the schema definition.

**NFS readiness pipeline specifics:**

The NFS readiness report (`scripts/nfs-readiness-report.sh`) classifies frontend plugins by their `backstage.features` content. The key domain constraint is:

- `rhdh-cli` populates `backstage.features` from module-federation entry points it detects during `export-dynamic-plugin`. A plugin with no NFS entry points ends up with an **empty** `backstage.features` field and is classified as `no-features` — not as `mixed` or `legacy-only`. Those statuses require a non-empty field containing non-NFS types, which the CLI does not produce in practice.
- The reachable statuses from the current pipeline are: `nfs-ready`, `no-features`, `unknown`, and `backend-only`. The statuses `mixed` and `legacy-only` are structurally unreachable given how `rhdh-cli` populates the field (see the "Reading the counts" note in the report's own markdown output).
- When reviewing NFS gate scripts, check the actual status distribution in the latest readiness report (generated by the `nfs-readiness-report.yaml` workflow) before accepting that blocking logic is reachable. If a gate blocks on `mixed` or `legacy-only` alone, it will never fire.

Future changes to `rhdh-cli` (e.g., adding `nfsFeaturesExposed` per RHIDP-16463) may expand the set of reachable statuses. When that happens, gates should be re-evaluated against the updated data.
