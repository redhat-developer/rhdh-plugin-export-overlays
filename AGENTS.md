...
**Key rules:**
- The `key` (first argument) must be **globally unique** across all spec files and projects. Prefix with the workspace name — and when the setup belongs to one project, **end it with the namespace**: `` `tech-radar-setup-${rhdh.deploymentConfig.namespace}` ``. The flag file is keyed on the key string alone, in a directory shared by every project in the run, so a literal key lets the first project's setup satisfy the second — which then deploys nothing and fails much later on a missing element. `deploy()` is unaffected only because its own key carries the namespace. A literal key is right when the setup really is shared, such as installing an operator into a fixed namespace every project then uses; `scripts/check_runonce_keys.py` enforces the distinction in CI; record a deliberately shared key in `scripts/runonce-shared-keys.txt` (or pass `--allow`).
