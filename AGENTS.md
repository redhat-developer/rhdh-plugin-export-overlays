...
**On `main`, migrating a workspace to the new frontend system (NFS) replaces its legacy Playwright project. It does not add a second one beside it.** `main` is the development line for RHDH 2.1, and RHIDP-15077 removes the legacy frontend from that release outright. The cut-over is redhat-developer/rhdh#5232: it deletes `packages/app-next/package.json` after renaming that package onto `packages/app`, and drops `ENABLE_STANDARD_MODULE_FEDERATION` entirely. There is no interim package and no opt-in path back. A legacy lane on `main` therefore spends a full cluster claim testing a configuration 2.1 will not ship. Once the cut-over lands it gets worse: both lanes boot the same shell, so the pair becomes two identical runs that no report tells apart.

**Legacy coverage lives on the release branches, not in a second lane.** `release-1.10` and `release-1.9` carry these suites against the RHDH versions that actually ship the legacy shell — 23 of the 24 suites on `main` are already there. Never migrate a lane on a release branch.

**Only drop the legacy lane if the NFS lane replacing it runs in nightly.** A lane carrying `test.skip(!!process.env.E2E_NIGHTLY_MODE, ...)` contributes nothing to the nightly, so swapping a running legacy lane for it deletes coverage instead of moving it. `app-defaults` is the live example: its only project is `app-defaults-app-next` and the whole suite is nightly-skipped on RHIDP-15482, so the workspace has no nightly coverage at all. When the NFS lane has to be nightly-skipped, keep both lanes and link the ticket that unblocks it.

**Two mechanisms are in use, and the choice is not settled here.** A lane switches to NFS either by renaming its project to `<name>-app-next` or by passing `configure({ useNewFrontendSystem: true })` in the spec. The rename is visible in the namespace, the reports and any inventory; the in-spec call is not, so `backstage`'s thirteen projects are NFS while every name still reads as legacy. That is a real cost, and it is also about to expire — once NFS is the only shell there is nothing for a suffix to distinguish. Deciding between them is RHIDP-16461. What matters for this section is only that both replace the legacy lane rather than adding to it.

**To restore a dropped lane**, revert the migration PR's `playwright.config.ts` hunk, or lift the project block from `release-1.10`, where the suites remain intact.
