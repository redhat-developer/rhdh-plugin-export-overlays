...
**On `main`, migrating a workspace to the new frontend system renames its Playwright project to `<name>-app-next`. It does not add a second project.** `main` is the development line for RHDH 2.1, and RHIDP-15077 removes the legacy frontend from that release outright — `packages/app` is deleted, `packages/app-next` takes its place, and there is no interim package and no opt-in path back. A legacy lane on `main` therefore spends a full cluster claim testing a configuration 2.1 will not ship. Once the cut-over lands it gets worse: both lanes boot the same shell, so the pair becomes two identical runs that no report tells apart.
...
**Prefer the rename over `configure({ useNewFrontendSystem: true })`.** Both switch the lane to NFS, but only the rename shows up in the project name — which is what the namespace, the reports and every inventory key on. Turning NFS on inside a spec while keeping the legacy project name replaces the lane invisibly. Settling on a single mechanism is RHIDP-16461; until that lands, rename.
