...
What remains ambiguous — no owner, or an owner that does not ship the file — is dropped rather than guessed, since misattributing one plugin's coverage to another is worse than losing it. Measured after the tie-break, both published workspaces drop nothing: `intelligent-assistant` resolves 105 of 105 and `adoption-insights` 45 of 45.
