...
Codecov *appends* a session per upload rather than replacing, so the scheduled backstop skips a commit that already carries every workspace it would upload — otherwise an unmoved `main` HEAD would collect duplicate sessions every 6h for no new data. Push and manual dispatch never skip, so re-running by hand always re-uploads. The check fails open: if the Codecov API is unreachable, it seeds rather than risk stranding a flag.
