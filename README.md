...
**This needs its own secret.** Codecov upload tokens are per project, so the workflow reads `CODECOV_UPSTREAM_TOKEN` — the *source* repo's token — not the `CODECOV_TOKEN` the anchor path uses. Without it the job fails fast rather than uploading nothing behind a green check.
